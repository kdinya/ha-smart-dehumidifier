  ...
  static styles = css`
    ...
    ha-card {
      ...
      /* Ширину картки повністю визначає розкладка HA (Masonry-колонка чи
         ...
         min-height - непорушна нижня межа: захищає лише СПРАВДІ вузькі
         (портретний телефон) екрани від зависокої, силоміць витягнутої
         по горизонталі картки. Важливо: тут НАВМИСНО НЕ layout_base_width
         ("Макс. ширина ПРИЛАДУ", до 1000px) - той параметр лишається
         виключно "стелею" розміру самого приладу (.dh-device нижче), а
         тут потрібне окреме, значно менше "вузьке" опорне значення: якщо
         взяти для підлоги layout_base_width за замовчуванням (510px),
         вона плато на ~485px висоти і продовжує домінувати аж до ширини
         картки ~825px (510*0.952*1.7) - тобто на типовому горизонтальному
         телефоні (~650-800px) підлога й далі перемагає aspect-ratio, і
         налаштована "Пропорція скла" візуально не встигає застосуватись
         (це й був баг). 320px - опорна ширина, підібрана так, щоб перетин
         з aspect-ratio (glass_aspect_ratio=1.7 за замовчуванням) припадав
         десь на ~520px: портретні телефони (~350-430px) - все ще чітко
         нижче цієї межі (підлога захищає форму як і раніше), а
         горизонтальні телефони й планшети (>=520px) - вже вище (перемагає
         налаштована пропорція скла, як і мало бути). Сам прилад
         (.dh-device нижче) і без цього ніколи не обрізається і не
         спотворюється - його власна формула (min(cqi, cqb*ratio, max))
         сама коректно вписує його в будь-яку висоту, яку віддасть картка.
      ...
      min-height: calc(
        min(100cqi, 320px) * var(--dh-frame-ar-num, 1)
    ...
    .dh-device {
      ...
      /* Єдина, безбрейкпоінтна формула "вписати найбільший прямокутник
         ...
         дозволяє ширина контейнера (100cqi - на всю доступну ширину, з
         урахуванням відступів, які вже враховані в .dh-scene padding),
         ширина, еквівалентна тому, що дозволяє ВИСОТА контейнера з
         урахуванням форми пристрою (100cqb * --dh-frame-ar-num), і
         налаштований максимум. Яке з трьох менше - те й перемагає, тож
         пристрій завжди повністю поміщається і по ширині, і по висоті,
         ніколи не обрізається і ніколи не зростає понад свій налаштований
         максимум. aspect-ratio виводить висоту з цієї ширини - форма
         пристрою завжди відповідає card_height_percent, а не примусово
         квадратна.

         ВАЖЛИВО: calc(cqb * --dh-frame-ar-num) математично ЗАВЖДИ
         дорівнює просто ширині самої .dh-frame-aspect (бо
         --dh-frame-ar-num = 100/card_height_percent, а cqb за
         побудовою = ширина рамки * card_height_percent/100 - відсоток
         скорочується). Тобто цей доданок - НЕ "запас по висоті", а
         рівно та сама ширина рамки, тільки БЕЗ віднімання бічних
         відступів (їх враховує лише 100cqi-доданок вище). Якщо раніше
         тут лишалось 95%, а бічні відступи (padLeft+padRight) сумарно
         менші за ці 5% ширини рамки - саме цей доданок (а не 100cqi)
         перемагав і обрізав пристрій ДО повної ширини картки, лишаючи
         порожній простір (майже весь - праворуч, бо вирівнювання за
         замовчуванням "left"). Замінено на 100%, щоб реальний запас
         по ширині завжди йшов ЛИШЕ від налаштованих відступів
         (content_padding_left/right), а не від випадкового
         додаткового 5%. Пристрій і далі ніколи не обрізається по
         висоті: коли .dh-frame-aspect реально стиснута через брак
         висоти (flex-shrink вище), її РЕАЛЬНА (вже стиснута) висота
         так само коректно відображається в cqb. */
